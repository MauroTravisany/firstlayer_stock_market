...
class Wp04SqlContractTests(unittest.TestCase):
    ...
    def test_reconciliation_and_fx_fail_closed(self):
        ...
        self.assertIn('provider = "BCCH_BDE"', fx)
        self.assertIn('series_id = "F073.TCO.PRE.Z.D"', fx)
        self.assertIn("source_rate_revision_id", fx)
        self.assertIn("first_observed_at", fx)
        self.assertIn('availability_policy = "wp04-bcch-vintage-availability-v2"', fx)
        self.assertIn("available_at = first_observed_at", fx)
        self.assertIn("NOT backtest_eligible", fx)
        self.assertNotIn("ROW_NUMBER()", fx)
        self.assertNotIn("latest_official_revision", fx)
