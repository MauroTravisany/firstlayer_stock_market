...
from packages.market_data.canonical import canonical_checksum, canonicalize_crypto_4h, canonicalize_session, total_return
...
def obs(*, start, interval="15m", open_=100, high=101, low=99, close=100, adjusted=None, volume=10, run="run-a", provider="YAHOO", asset="STOCK", exchange="XNYS"):
    ...
    return RawPriceObservation(
        ...
        adjusted_close=adjusted, volume=volume, currency="USD",
...
class MarketDataCoreTests(unittest.TestCase):
    def test_content_addressed_ids_are_stable_and_revision_sensitive(self):
        ...
        first = obs(start=start, run="run-1")
        same = obs(start=start, run="run-2")
        changed = obs(start=start, close=101, high=102, run="run-3")
    ...
    def test_observation_cannot_be_available_before_bar_close(self):
        ...
        with self.assertRaisesRegex(ValueError, "available_at"):
            RawPriceObservation(
                provider="Y", provider_symbol="AAPL", ticker="AAPL", asset_type="STOCK", exchange="XNYS",
                source_interval="15m", source_timezone="UTC", bar_start=start, bar_end=start+dt.timedelta(minutes=15),
                open=100, high=101, low=99, close=100, adjusted_close=100, volume=1, currency="USD",
                available_at=start, ingested_at=start+dt.timedelta(minutes=16), ingestion_run_id="r", source_version="v", payload={}
            )
    ...
    def test_fx_calendar_is_weekday_24h_utc(self):
        session = session_for_date(
            dt.date(2026, 8, 14), exchange="FX_24_5", asset_type="FX"
        )
        self.assertEqual(0, session.open_utc.hour)
        self.assertEqual(24 * 60 * 60, int((session.close_utc - session.open_utc).total_seconds()))
        self.assertIsNone(
            session_for_date(
                dt.date(2026, 8, 15), exchange="FX_24_5", asset_type="FX"
            )
        )
    ...
    def test_daily_and_intraday_never_mix(self):
        ...
        rows = []
        for i in range(expected_bars(session, "15m")):
            start = session.open_utc + dt.timedelta(minutes=15*i)
            rows.append(obs(start=start, open_=100+i/100, high=101+i/100, low=99+i/100, close=100.5+i/100, volume=10))
        rows.append(obs(start=dt.datetime(2026,8,13,tzinfo=UTC), interval="1d", open_=95, high=110, low=90, close=105, adjusted=104, volume=9999))
    ...
    def test_incomplete_intraday_falls_back_to_daily(self):
        session = session_for_date(dt.date(2026, 8, 13))
        rows = [obs(start=session.open_utc), obs(start=dt.datetime(2026,8,13,tzinfo=UTC), interval="1d", open_=98, high=103, low=97, close=102, adjusted=101, volume=500)]
        bar = canonicalize_session(rows, session)
        self.assertEqual("DAILY_FALLBACK_INCOMPLETE_INTRADAY", bar.selected_by_rule)
    ...
    def test_crypto_4h_requires_four_complete_hourly_bars(self):
        ...
        complete = [obs(start=start+dt.timedelta(hours=i), interval="1h", asset="CRYPTO", exchange="CRYPTO_24_7", open_=100+i, high=101+i, low=99+i, close=100.5+i) for i in range(4)]
    ...
    def _bar(self, provider, close, volume=100, available_offset=0):
        start = dt.datetime(2026,8,13,13,30,tzinfo=UTC)
        return CanonicalBar(
            ticker="AAPL", asset_type="STOCK", exchange="XNYS", session_date=dt.date(2026,8,13),
            bar_start=start, bar_end=start+dt.timedelta(hours=6,minutes=30), canonical_interval="1d",
            raw_open=100, raw_high=max(101,close), raw_low=99, raw_close=close, adjusted_close=close,
            raw_volume=volume, currency="USD", provider=provider, source_interval="1d", coverage_ratio=1.0,
            quality_status="CANONICAL_RECONCILED", selected_by_rule="DAILY_FALLBACK_NO_INTRADAY",
            available_at=start+dt.timedelta(hours=7,seconds=available_offset), source_revision_ids=(provider,),
        )

    def test_provider_reconciliation_blocks_material_mismatch(self):
        result = reconcile_bars(self._bar("YAHOO",100), self._bar("STOOQ",102), close_tolerance_bps=10)
    ...
    def test_provider_reconciliation_accepts_small_difference(self):
        result = reconcile_bars(self._bar("YAHOO",100), self._bar("STOOQ",100.05), close_tolerance_bps=10)
    ...
    def test_canonical_checksum_is_order_independent(self):
        a = self._bar("YAHOO",100)
        b = CanonicalBar(**{**a.__dict__, "ticker":"MSFT", "source_revision_ids":("MSFT",)})
        self.assertEqual(canonical_checksum([a,b]), canonical_checksum([b,a]))
