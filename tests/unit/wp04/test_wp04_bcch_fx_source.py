...
class Wp04BcchFxSourceTests(unittest.TestCase):
    ...
    def test_official_rows_preserve_scalar_rate_and_publication_lineage(self):
        ...
        self.assertEqual(dt.datetime(2024, 10, 4, 20, 30, tzinfo=UTC), first["available_at"])
        self.assertEqual(first["source_published_at"], first["available_at"])
        self.assertEqual("OFFICIAL_PUBLISHED_RATE", first["quality_status"])
