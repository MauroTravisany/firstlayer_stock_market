...
def build_queries(project_id: str, dataset_id: str, mapping_version: str):
    ...
    context = fqtn(project_id, dataset_id, "trading_earnings_context_pit")
    valuation = fqtn(project_id, dataset_id, "portfolio_valuation_pit_shadow")
    audit = fqtn(project_id, dataset_id, "audit_no_lookahead")
    ...
    queries = {
        "raw_summary": f"""
          SELECT COUNT(*) AS row_count,
            ...
            MIN(available_at) AS min_available_at, MAX(available_at) AS max_available_at,
            MIN(loaded_at) AS min_loaded_at, MAX(loaded_at) AS max_loaded_at
          FROM {raw}
        """,
        "canonical_summary": f"""
          SELECT COUNT(*) AS row_count,
        ...
        "quarter_summary": f"""
          SELECT COUNT(*) AS row_count,
            ...
            COUNTIF(derivation_component_max_available_at IS NOT NULL AND derivation_component_max_available_at > available_at) AS q4_component_after_annual_count,
            COUNTIF(derivation_component_max_available_at IS NOT NULL AND derivation_component_max_available_at > signal_timestamp) AS q4_component_after_signal_count,
            COUNTIF(currency IS NULL) AS missing_currency_count
          FROM {quarters}
        """,
        "ttm_summary": f"""
          SELECT COUNT(*) AS row_count, COUNTIF(ttm_complete) AS complete_row_count,
            COUNTIF(ttm_complete AND (quarter_count != 4 OR distinct_quarter_count != 4 OR quarter_ordinal_span != 3 OR currency_count != 1 OR normalized_quarter_count != 4 OR revenue_quarter_count != 4 OR net_income_quarter_count != 4)) AS invalid_complete_row_count,
        ...
        "earnings_context_summary": f"""
          SELECT COUNT(*) AS row_count,
            ...
            COUNTIF(earnings_event_status LIKE 'RECENT_%' AND (reported_age_days IS NULL OR reported_age_days NOT BETWEEN 0 AND 5)) AS stale_recent_status_count
          FROM {context}
        """,
        "valuation_summary": f"""
          SELECT COUNT(*) AS row_count,
        ...
        "comparison_summary": f"""
          SELECT COALESCE(SUM(row_count), 0) AS row_count,
            ...
            ARRAY_AGG(STRUCT(pit_divergence_type, row_count) ORDER BY pit_divergence_type) AS status_counts
          FROM (
            SELECT pit_divergence_type, promotion_eligible, COUNT(*) AS row_count
            FROM {comparison}
...
def _nonzero_problems(result, pairs):
    return [
        {"code": code, "count": _count(result, key)}
        for key, code in pairs
        if _count(result, key)
    ]


def evaluate_results(*, query_results, raw_schema_drift):
    required = {
        "raw_summary", "canonical_summary", "quarter_summary", "ttm_summary",
        "earnings_summary", "earnings_context_summary", "valuation_summary",
        "no_lookahead_summary", "comparison_summary", "invalidation_summary",
    }
    ...
    audit = query_results["no_lookahead_summary"]
    ...
    if _count(raw, "duplicate_revision_count"):
        problems.append({"code": "RAW_DUPLICATE_REVISION", "count": _count(raw, "duplicate_revision_count")})
    ...
    if _count(raw, "other_mapping_row_count"):
        warnings.append({"code": "RAW_RETAINS_OTHER_MAPPING_VERSIONS", "count": _count(raw, "other_mapping_row_count")})

    if _count(canonical, "row_count") <= 0:
        problems.append({"code": "CANONICAL_PIT_EMPTY"})
    problems += _nonzero_problems(canonical, (
    ...
    problems += _nonzero_problems(quarters, (
    ...
    if _count(quarters, "incomplete_q4_count"):
        warnings.append({"code": "INCOMPLETE_Q4_ROWS_RETAINED_FAIL_CLOSED", "count": _count(quarters, "incomplete_q4_count")})

    if _count(ttm, "row_count") <= 0:
        problems.append({"code": "TTM_PIT_EMPTY"})
    problems += _nonzero_problems(ttm, (
    ...
    problems += _nonzero_problems(earnings, (
    ...
    problems += _nonzero_problems(earnings_context, (
    ...
    problems += _nonzero_problems(valuation, (
    ...
    if _count(audit, "violation_count"):
        problems.append({"code": "NO_LOOKAHEAD_VIOLATION", "count": _count(audit, "violation_count")})
    ...
    if _count(comparison, "promotion_violation_count"):
        problems.append({"code": "COMPARISON_PROMOTION_ELIGIBLE", "count": _count(comparison, "promotion_violation_count")})
    if _count(invalidation, "row_count") <= 0:
        problems.append({"code": "LEGACY_INVALIDATION_EMPTY"})
    problems += _nonzero_problems(invalidation, (
...
def capture_evidence(*, project_id, dataset_id, location, mapping_version, expected_git_sha, ci_run_id):
    ...
    if actual_location and actual_location.lower() != location.lower():
        raise EvidenceError(f"dataset location mismatch: expected {location}, actual {actual_location}")
    ...
    for name in REQUIRED_TABLES:
        ...
        metadata[name] = {
            ...
            "schema_sha256": sha256_json(snapshot),
        }
    if missing:
        raise EvidenceError(f"missing required WP-03 shadow tables: {missing}")

    raw_table = client.get_table(f"{project_id}.{dataset_id}.financial_statements_pit_raw")
    contract = load_contract(REPO_ROOT / "contracts" / "financial_statements_pit_raw.yaml")
    drift = compare_schema(contract, _schema(raw_table), allow_extra_columns=False)
    results = {}
    hashes = {}
    job_ids = {}
    for name, sql in build_queries(project_id, dataset_id, mapping_version).items():
        hashes[name] = hashlib.sha256(sql.encode("utf-8")).hexdigest()
        job = client.query(sql, job_config=bigquery.QueryJobConfig(use_legacy_sql=False), location=location)
        results[name] = _first_row(job)
        job_ids[name] = job.job_id

    evaluation = evaluate_results(query_results=results, raw_schema_drift=drift)
    ...
    stable = {
        ...
        "dataset_location": actual_location,
        "location": location,
    ...
    return {**stable, "evidence_checksum": sha256_json(stable), "captured_at": datetime.now(timezone.utc).isoformat()}


def write_evidence(document, output):
