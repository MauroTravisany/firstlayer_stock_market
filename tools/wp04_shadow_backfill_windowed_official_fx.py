"""Create a moving-window WP-04 plan with mandatory official BCCh FX."""
...
from tools import wp04_official_fx_planner as official
...
def build_windowed_plan(
    *,
    ...
    planner: Callable[..., dict[str, Any]] = official.build_plan,
    ...
    plan = planner(
        ...
        intraday_start_date=dt.date.fromisoformat(window["intraday_start_date"]),
...
def _write_plan(document: Mapping[str, Any], output: Path) -> None:
...
def main() -> int:
    ...
    try:
        ...
        bcch_token = os.environ.get(args.bcch_api_token_env)
        if not str(bcch_token or "").strip():
            raise backfill.Wp04BackfillError("AUTHENTICATION_MISSING")
        plan, window = build_windowed_plan(
            ...
            stooq_api_key=(
                os.environ.get(args.stooq_api_key_env) if args.stooq_api_key_env else None
            ),
            bcch_api_token=bcch_token,
        )
