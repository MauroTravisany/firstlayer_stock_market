...
| BASE-05 | Política permanece `SHADOW_ONLY`, Brain pausado o `LEGACY_RESEARCH`, cambios productivos falsos y Alpaca Paper | Habilitación accidental de ejecución | WP-00 | validación fail-closed y blocker operativo en [`tools/capture_baseline.py`](../../tools/capture_baseline.py) | invariantes de policy/Alpaca (exit 0); manifest observa `strategy-brain-generate` y `strategy-brain-review` en `ENABLED` | [`baseline_manifest.json`](evidence/baseline_manifest.json) | FAIL: requiere autorización para pausar schedulers y recapturar | 2026-08-08 |
