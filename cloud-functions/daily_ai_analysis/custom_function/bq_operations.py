...
def fetch_daily_signals(config, analysis_date, tickers=None, analysis_scope="candidates", max_tickers=None):
    ...
    SELECT
      ...
      {_select_expr("earnings_date", available, "DATE")},
      {_select_expr("days_to_earnings", available, "INT64")},
      {_select_expr("earnings_time", available, "STRING")},
      {_select_expr("eps_estimate", available)},
      {_select_expr("reported_eps", available)},
      {_select_expr("surprise_pct", available)},
      {_select_expr("earnings_event_status", available, "STRING")},
      {_select_expr("earnings_score", available)},
      {_select_expr("earnings_context_note", available, "STRING")},
      {_select_expr("ratios_may_be_stale_after_earnings", available, "BOOL")},
      {_select_expr("earnings_data_freshness_note", available, "STRING")},
